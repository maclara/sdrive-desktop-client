...
public:
    ...
    SetContentsCommand(const QString &fileName, unsigned count, char ch)
        : Command(fileName)
        , _count(count)
        , _ch(ch)
    {
...
public:
    ...
    bool execute(QDir &rootDir) const override
    {
        ...
        cerr << ".... file: " << qPrintable(_fileName) << ", byte: " << ch << ", size: " << f.size() << endl;
        if (!f.seek(f.size())) {
...
public:
    ...
    InsertCommand(const QString &fileName, int count, char ch)
        : Command(fileName)
        , _count(count)
        , _ch(ch)
    {
